...
function Dashboard() {
  ...
  // Prevent back-button from going to login/signup while authenticated
  useEffect(() => {
    // Only enable this protection if we have a token
    if (!token) return;

    // Push a new history entry so the first back goes to this state
    try {
      window.history.pushState(null, '', window.location.href);
    } catch (e) {
      // ignore (some browsers may restrict pushState in some contexts)
    }

    const onPop = (ev) => {
      const p = window.location.pathname.replace(/\/$/, '');
      // common auth routes in this app: root ("/"), /login, /signup
      if (p === '' || p === '/' || p === '/login' || p === '/signup') {
        // immediately put user back on the dashboard route
        try {
          window.history.pushState(null, '', '/dashboard');
        } catch (e) {
          // fallback: force location (will reload)
  ...
  // Remove a keyword from a note (updates backend and refreshes notes)
      ...
      <div className="content">
        ...
        {activeTab === "home" && (
          <div className="home-tab">
            ...
            {/* Keywords bar: show popular keywords outside of cards */}
            ...
            <div className="recent-notes note-grid">
              {notes.map(note => (
                <div key={note._id} className="note-card">
                  ...
                  {/* Show only a preview of the content (first 100 chars, plain text) */}
                  <div className="note-content" dangerouslySetInnerHTML={{ __html: highlightText(note.content.replace(/<[^>]+>/g, "").slice(0, 100) + (note.content.replace(/<[^>]+>/g, "").length > 100 ? '...' : ''), searchQuery) }} />
                  {/* Optionally, add a 'View' button for future modal/full view */}
                  {note.keywords && note.keywords.length > 0 && (
                    <div className="note-keywords">
                      {note.keywords.map((k, i) => (
                        <span key={i} className="keyword">
                          {k}
                          <button type="button" className="kw-remove" onClick={(e) => { e.stopPropagation(); handleRemoveKeyword(note._id, k); }} aria-label={`Remove keyword ${k}`}>×</button>
                        </span>
                      ))}
                    </div>
                  )}
          ...
          <div className="notes-tab">
            ...
            {filteredNotes.length === 0 ? <p>No notes found.</p> : (
              ...
              {filteredNotes.map(note => (
                <div key={note._id} className={`note-card ${expandedNoteId === note._id ? 'expanded' : ''}`} onClick={() => setExpandedNoteId(expandedNoteId === note._id ? null : note._id)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setExpandedNoteId(expandedNoteId === note._id ? null : note._id); }}>
                  <h3 dangerouslySetInnerHTML={{ __html: highlightText(note.title, searchQuery) }} />
                  <div className="note-content" dangerouslySetInnerHTML={{ __html: highlightText(note.content.replace(/<[^>]+>/g, ""), searchQuery) }} />
                {note.keywords && note.keywords.length > 0 && (
                  <div className="note-keywords">
                      {note.keywords.map((k, i) => (
                        <span key={i} className="keyword">
