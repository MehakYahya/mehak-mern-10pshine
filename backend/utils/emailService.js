...
// Verify transporter immediately to surface issues in logs
transporter.verify().then(() => {
  console.log('✅ Email transporter verified (EMAIL_USER set?):', !!process.env.EMAIL_USER);
}).catch(err => {
...
const sendPasswordResetCodeEmail = async (email, code) => {
  const mailOptions = {
    from: process.env.EMAIL_USER,
    to: email,
    subject: "Your Password Reset Code - 10pShine Notes App",
    html: `
      <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: 0 auto;">
        ...
        <div style="text-align: center; margin: 30px 0;">
          <span style="font-size: 2em; letter-spacing: 8px; color: #4CAF50; font-weight: bold;">${code}</span>
        </div>
        <p style="color: #666; font-size: 14px;"><strong>This code will expire in 15 minutes.</strong></p>
        <p style="color: #666; font-size: 14px;">If you didn't request this, please ignore this email.</p>
        <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">
        <p style="color: #999; font-size: 12px;">© 2025 10pShine Notes App. All rights reserved.</p>
      </div>
    `,
  };
  ...
  try {
    const info = await transporter.sendMail(mailOptions);
    console.log("✅ Code email sent successfully:", info.messageId);
    return { success: true, messageId: info.messageId };
  } catch (error) {
    console.error("❌ Code email sending failed:", error && error.message ? error.message : error);
